...
use crate::{
    ...
    auth::{auth_manager::AuthManager, auth_state::AuthState, AuthStateProvider},
};
...
impl AccountWidget {
    fn render_local_account_info(&self, appearance: &Appearance) -> Box<dyn Element> {
        Flex::column()
            ...
            .with_child(
                Container::new(
                    appearance
                        .ui_builder()
                        .paragraph(
                            "Warp cloud account features are disabled in this local-first build."
                                .to_string(),
                        )
    ...
    fn render_account_info(
        ...
        Align::new(user_info.finish()).left().finish()
    }
}
...
impl SettingsWidget for AccountWidget {
    ...
    fn render(
        ...
        let account_info = if view.auth_state.is_anonymous_or_logged_out() {
            self.render_local_account_info(appearance)
        } else {
